 ...
 * @status
 */
public class CRC implements TalendJob {
	...
	private final String jobName = "CRC";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(CRC.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row3Struct implements routines.system.IPersistableRow<row3Struct> {
		final static byte[] commonByteArrayLock_KEYRUS_CRC = new byte[0];
		static byte[] commonByteArray_KEYRUS_CRC = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_KEYRUS_CRC.length) {
					if (length < 1024 && commonByteArray_KEYRUS_CRC.length == 0) {
						commonByteArray_KEYRUS_CRC = new byte[1024];
					} else {
						commonByteArray_KEYRUS_CRC = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_KEYRUS_CRC, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_CRC, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_KEYRUS_CRC, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_CRC, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_KEYRUS_CRC) {
		...
		public String toString() {
			...
			sb.append(",ddate=" + String.valueOf(ddate));
	...
	public void tRowGenerator_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_2.setTableName("LOG");
				util_tLogRow_2.addRow(new String[] { "code", "nom", "prenom", "ddate", "CRC", });
...
///////////////////////    			
				...
				util_tLogRow_1.setTableName("DATA");
				util_tLogRow_1.addRow(new String[] { "code", "nom", "prenom", "ddate", });
...
///////////////////////    			
				...
				class tRowGenerator_1Randomizer {
					...
					public java.util.Date getRandomddate() {
				...
				for (int itRowGenerator_1 = 0; itRowGenerator_1 < nb_max_row_tRowGenerator_1; itRowGenerator_1++) {
					...
					row1.ddate = randtRowGenerator_1.getRandomddate();
...
///////////////////////		
					...
					if (row1.code != null) { //
						row_tLogRow_1[0] = String.valueOf(row1.code);
					...
					if (row1.ddate != null) { //
						row_tLogRow_1[3] = FormatterUtils.format_Date(row1.ddate, "dd-MM-yyyy");
...
///////////////////////		
					...
					if (row3.code != null) { //
						row_tLogRow_2[0] = String.valueOf(row3.code);

					} //

					if (row3.nom != null) { //
						row_tLogRow_2[1] = String.valueOf(row3.nom);

					} //

					if (row3.prenom != null) { //
						row_tLogRow_2[2] = String.valueOf(row3.prenom);

					} //

					if (row3.ddate != null) { //
						row_tLogRow_2[3] = FormatterUtils.format_Date(row3.ddate, "dd-MM-yyyy");

					} //

					if (row3.CRC != null) { //
						row_tLogRow_2[4] = String.valueOf(row3.CRC);
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final CRC CRCClass = new CRC();

		int exitCode = CRCClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = CRC.class.getClassLoader()
					.getResourceAsStream("keyrus/crc_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = CRC.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : CRC");
		}
...
/************************************************************************************************
 * 73696 characters generated by Talend Open Studio for Data Integration on the
 * 31 mars 2022 à 18:45:09 CEST
 ************************************************************************************************/