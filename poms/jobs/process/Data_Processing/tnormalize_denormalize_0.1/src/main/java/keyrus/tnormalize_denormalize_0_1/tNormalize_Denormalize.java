...
package keyrus.tnormalize_denormalize_0_1;
...
/**
 * Job: tNormalize_Denormalize Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class tNormalize_Denormalize implements TalendJob {
	...
	private final String jobName = "tNormalize_Denormalize";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					tNormalize_Denormalize.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(tNormalize_Denormalize.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row4Struct implements routines.system.IPersistableRow<row4Struct> {
		final static byte[] commonByteArrayLock_KEYRUS_tNormalize_Denormalize = new byte[0];
		static byte[] commonByteArray_KEYRUS_tNormalize_Denormalize = new byte[0];
		...
		public String tretment_name;

		public String getTretment_name() {
			return this.tretment_name;
		}
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_KEYRUS_tNormalize_Denormalize.length) {
					if (length < 1024 && commonByteArray_KEYRUS_tNormalize_Denormalize.length == 0) {
						commonByteArray_KEYRUS_tNormalize_Denormalize = new byte[1024];
					} else {
						commonByteArray_KEYRUS_tNormalize_Denormalize = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_KEYRUS_tNormalize_Denormalize, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_tNormalize_Denormalize, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_KEYRUS_tNormalize_Denormalize, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_tNormalize_Denormalize, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_KEYRUS_tNormalize_Denormalize) {

				try {

					int length = 0;

					this.pet_name = readString(dis);

					this.pet_type = readString(dis);

					this.pet_owner = readString(dis);

					this.tretment_name = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.pet_name, dos);

				// String

				writeString(this.pet_type, dos);

				// String

				writeString(this.pet_owner, dos);

				// String

				writeString(this.tretment_name, dos);
		...
		public String toString() {
			...
			sb.append(",pet_owner=" + pet_owner);
			sb.append(",tretment_name=" + tretment_name);
			sb.append("]");
	...
	public void tFixedFlowInput_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_3.setTableName("main2");
				util_tLogRow_3.addRow(new String[] { "pet_name", "pet_type", "pet_owner", "tretment_name", });
...
///////////////////////    			
				...
				util_tLogRow_4.setTableName("result2");
				util_tLogRow_4.addRow(new String[] { "pet_name", "pet_type", "pet_owner", "tretment_name", });
...
///////////////////////    			
				...
				List<row3Struct> cacheList_tFixedFlowInput_2 = new java.util.ArrayList<row3Struct>();
				row3 = new row3Struct();
				row3.pet_name = "mimi";
				row3.pet_type = "cat";
				row3.pet_owner = "mehdi";
				row3.tretment_name = "vacc_steri_accc";
				cacheList_tFixedFlowInput_2.add(row3);
				row3 = new row3Struct();
				row3.pet_name = "roudi";
				row3.pet_type = "dog";
				row3.pet_owner = "mehdi";
				row3.tretment_name = "t1_t2";
				cacheList_tFixedFlowInput_2.add(row3);
				for (int i_tFixedFlowInput_2 = 0; i_tFixedFlowInput_2 < 1; i_tFixedFlowInput_2++) {
					for (row3Struct tmpRow_tFixedFlowInput_2 : cacheList_tFixedFlowInput_2) {
						nb_line_tFixedFlowInput_2++;
						row3 = tmpRow_tFixedFlowInput_2;
						...
						row7.pet_name = row3.pet_name;
						row7.pet_type = row3.pet_type;
						row7.pet_owner = row3.pet_owner;
						row7.tretment_name = row3.tretment_name;
						row8 = new row8Struct();

						row8.pet_name = row3.pet_name;
						row8.pet_type = row3.pet_type;
						row8.pet_owner = row3.pet_owner;
						row8.tretment_name = row3.tretment_name;
...
///////////////////////		
						...
						if (row7.pet_name != null) { //
							row_tLogRow_3[0] = String.valueOf(row7.pet_name);

						} //

						if (row7.pet_type != null) { //
							row_tLogRow_3[1] = String.valueOf(row7.pet_type);

						} //

						if (row7.pet_owner != null) { //
							row_tLogRow_3[2] = String.valueOf(row7.pet_owner);

						} //

						if (row7.tretment_name != null) { //
							row_tLogRow_3[3] = String.valueOf(row7.tretment_name);
...
///////////////////////    			
						...
						if (row8.tretment_name != null) {
							if ("".equals(row8.tretment_name)) {
								normalizeRecord_tNormalize_1[0] = "";
							} else {

								normalizeRecord_tNormalize_1 = row8.tretment_name.split("_", -1);
						...
						for (int i_tNormalize_1 = 0; i_tNormalize_1 < lastNoEmptyIndex_tNormalize_1; i_tNormalize_1++) {
							...
							row4.pet_name = row8.pet_name;

							row4.pet_type = row8.pet_type;

							row4.pet_owner = row8.pet_owner;

							row4.tretment_name = normalizeRecord_tNormalize_1[i_tNormalize_1];
...
///////////////////////		
							...
							if (row4.pet_name != null) { //
								row_tLogRow_4[0] = String.valueOf(row4.pet_name);

							} //

							if (row4.pet_type != null) { //
								row_tLogRow_4[1] = String.valueOf(row4.pet_type);

							} //

							if (row4.pet_owner != null) { //
								row_tLogRow_4[2] = String.valueOf(row4.pet_owner);

							} //

							if (row4.tretment_name != null) { //
								row_tLogRow_4[3] = String.valueOf(row4.tretment_name);
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final tNormalize_Denormalize tNormalize_DenormalizeClass = new tNormalize_Denormalize();

		int exitCode = tNormalize_DenormalizeClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = tNormalize_Denormalize.class.getClassLoader()
					.getResourceAsStream("keyrus/tnormalize_denormalize_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = tNormalize_Denormalize.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : tNormalize_Denormalize");
		}
...
/************************************************************************************************
 * 79813 characters generated by Talend Open Studio for Data Integration on the
 * 31 mars 2022 à 18:32:51 CEST
 ************************************************************************************************/