...
package keyrus.tunpivotrow_0_1;
 ...
 * @status
 */
public class tunpivotRow implements TalendJob {
	...
	private final String jobName = "tunpivotRow";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(tunpivotRow.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row5Struct implements routines.system.IPersistableRow<row5Struct> {
		final static byte[] commonByteArrayLock_KEYRUS_tunpivotRow = new byte[0];
		static byte[] commonByteArray_KEYRUS_tunpivotRow = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_KEYRUS_tunpivotRow.length) {
					if (length < 1024 && commonByteArray_KEYRUS_tunpivotRow.length == 0) {
						commonByteArray_KEYRUS_tunpivotRow = new byte[1024];
					} else {
						commonByteArray_KEYRUS_tunpivotRow = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_KEYRUS_tunpivotRow, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_tunpivotRow, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_KEYRUS_tunpivotRow, 0, length);
				strReturn = new String(commonByteArray_KEYRUS_tunpivotRow, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_KEYRUS_tunpivotRow) {
	...
	public void tFixedFlowInput_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_3.addRow(new String[] { "pivot_key", "pivot_value", "code", });
...
///////////////////////    			
				...
				for (int i_tFixedFlowInput_2 = 0; i_tFixedFlowInput_2 < 1; i_tFixedFlowInput_2++) {
					for (row4Struct tmpRow_tFixedFlowInput_2 : cacheList_tFixedFlowInput_2) {
						 ...
						 */
						String[] pivotKeysArray = { "nom" };

						for (String field : pivotKeysArray) {
							row5.code = row4.code;

							row5.pivot_key = field;
							try {
								row5.pivot_value = row4.getClass().getDeclaredField(field).get(row4).toString();
							} catch (Exception e) {
								row5.pivot_value = null;
							}
							...
							/**
							 * [tUnpivotRow_2 process_data_begin ] start
							 */

							currentComponent = "tUnpivotRow_2";

							/**
							 * [tUnpivotRow_2 process_data_begin ] stop
							 */
...
///////////////////////		
							...
							if (row5.pivot_key != null) { //
								row_tLogRow_3[0] = String.valueOf(row5.pivot_key);

							} //

							if (row5.pivot_value != null) { //
								row_tLogRow_3[1] = String.valueOf(row5.pivot_value);

							} //

							if (row5.code != null) { //
								row_tLogRow_3[2] = String.valueOf(row5.code);
...
///////////////////////    			
						...
						/**
						 * [tUnpivotRow_2 process_data_end ] start
						 */

						currentComponent = "tUnpivotRow_2";

						/**
						 * [tUnpivotRow_2 process_data_end ] stop
						 */
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final tunpivotRow tunpivotRowClass = new tunpivotRow();

		int exitCode = tunpivotRowClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = tunpivotRow.class.getClassLoader()
					.getResourceAsStream("keyrus/tunpivotrow_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = tunpivotRow.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : tunpivotRow");
		}
...
/************************************************************************************************
 * 48789 characters generated by Talend Open Studio for Data Integration on the
 * 13 mai 2022 à 17:04:41 CEST
 ************************************************************************************************/